  ...
  return (
    <div className="grid grid-cols-1 gap-4 px-4 *:data-[slot=card]:bg-gradient-to-t *:data-[slot=card]:from-primary/5 *:data-[slot=card]:to-card *:data-[slot=card]:shadow-xs lg:px-6 @xl/main:grid-cols-2 dark:*:data-[slot=card]:bg-card">
      <Card className="@container/card">
        <CardHeader>
          <CardDescription>Nowi członkowie</CardDescription>
        ...
        <CardFooter className="flex-col items-start gap-1.5 text-sm">
          ...
          <div className="text-muted-foreground">
            {isLoading
              ? "Wczytywanie danych..."
              : `Poprzedni miesiąc: ${newMembers?.previous_month_count ?? 0}`}
          </div>
        </CardFooter>
      </Card>
      <Card className="@container/card">
        <CardHeader>
          <CardDescription>Obłożenie grup</CardDescription>
          <CardTitle className="text-2xl font-semibold tabular-nums @[250px]/card:text-3xl">
          ...
          <CardAction>
            <Badge variant="outline">
              <RiArrowRightLine />
              {isLoading ? "..." : `${occupancySessions} sesji`}
            </Badge>
          </CardAction>
        </CardHeader>
        <CardFooter className="flex-col items-start gap-1.5 text-sm">
          <div className="line-clamp-1 flex gap-2 font-medium">
            Frekwencja względem dostępnych miejsc
          </div>
          <div className="text-muted-foreground">
            {isLoading
              ? "Wczytywanie danych..."
              : `${occupancyAttended} obecności / ${occupancyCapacity} miejsc`}
          </div>
        </CardFooter>
      </Card>
    </div>
